var GrubbsCalculator = {
  ...
  calculateGrubbsValue: function (size, probabilty) {
    ...
    return (
      (size - 1) * (tInvVal / Math.sqrt(size * (size - 2 + tInvVal * tInvVal)))
    );
  },
  ...
  calculateAutoCovariance: function (range1, range2) {
    ...
    var range1Average = range1.reduce(function (acc, obj) {
      return acc + obj.Value;
    }, 0);
    range1Average = range1Average / range1.length;
    var range2Average = range2.reduce(function (acc, obj) {
      return acc + obj.Value;
    }, 0);
    ...
    for (var i = 0; i < size; i++) {
      if (typeof range2[i] !== "undefined") {
        var product =
          (range1[i].Value - range1Average) * (range2[i].Value - range2Average);
      }
      sumProduct += product;
    }
  ...
  identifyComponentOutliers: function (
    samples,
    grubbsValue,
    average,
    standardDeviation,
