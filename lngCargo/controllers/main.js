angular
  .module("GCDarseReplacementApp", ["ngMaterial", "ngMessages"])
  .controller("mainCtrl", function ($scope) {
    ...
    this.flowLimit = 4000;
    this.removeFlowUnderLimit = false;

    var debug = false;

    if (debug) {
      this.cargoName = "TEST";
      this.startDate = new Date("2024-03-27 3:33 PM");
      this.endDate = new Date("2024-03-28 11:00 AM");
      this.startTime = new Date("1970-01-01 3:33 PM");
      this.endTime = new Date("1970-01-01 11:00 AM");
      this.jetty = "South";
      this.analyzer = "South";
      this.removeFlowUnderLimit = true;
      this.flowLimit = 8000;
    }
    ...
    var analyzerTags = [
      "CAM_DCS:AI1001A_C6",
      "CAM_DCS:AI1001A_ETH",
      "CAM_DCS:AI1001A_IPEN",
      "CAM_DCS:AI1001A_IBU",
      "CAM_DCS:AI1001A_METH",
      "CAM_DCS:AI1001A_N2",
      "CAM_DCS:AI1001A_NPEN",
      "CAM_DCS:AI1001A_NBU",
      "CAM_DCS:AI1001A_PRO",
      "CAM_DCS:AI1001A_CO2",
      "CAM_DCS:AI1001B_C6",
      "CAM_DCS:AI1001B_ETH",
      "CAM_DCS:AI1001B_IPEN",
      "CAM_DCS:AI1001B_IBU",
      "CAM_DCS:AI1001B_METH",
      "CAM_DCS:AI1001B_N2",
      "CAM_DCS:AI1001B_NPEN",
      "CAM_DCS:AI1001B_NBU",
      "CAM_DCS:AI1001B_PRO",
      "CAM_DCS:AI1001B_CO2",
      "CAM_DCS:AI1101A9",
    ...
    async function getData(start, end) {
      ...
      for (const tag of jettyTags) {
        const interpolatedData = await getInterpolatedData(
          tag.WebID,
          start,
          end,
          interval
    ...
    this.ClickFlowLimitSwitch = function () {
      console.log(
        "Keep Flow < " + this.flowLimit + "gpm: " + this.removeFlowUnderLimit
      );
    };
    ...
    this.IWantData = function () {
      ...
      getData(oneHourBeforeStart, oneHourAfterEnd)
        .then((data) => {
          ...
          cleanDataSamples();
          copyFlowSamples = dataSamples.filter(function (sample) {
          ...
          console.log(
            "Keep Flow  under " +
              this.flowLimit +
              " gpm?: " +
              this.removeFlowUnderLimit
          );
          if (this.removeFlowUnderLimit) {
            removeJunk(this.flowLimit);
          }
        ...
        .then(function () {
          console.log(
            "Gathering samples from within Loading Window (start-end)..."
          );
          dataSamples.forEach(function (set) {
            var samplesInWindow = getLoadingSamples(
              set.samples,
              startISO,
              endISO
            );
          ...
          performGrubbsOnFlow();
          console.log(
            "...outlier count after flow(autocovarience): " + outliers.length
          );
        })
        .then(function () {
        ...
        .then(function () {
          ...
          for (var i = 0; i < rawFlowTotals.length; i++) {
            const found = copyFlowResultingSamples.some(
              (el) => el.Timestamp === rawFlowTotals[i].Timestamp
            );
            if (!found) {
              if (
                rawFlowTotals[i].Timestamp >= startISO &&
                rawFlowTotals[i].Timestamp <= endISO
              ) {
                var sample = {
        ...
        .then(function () {
          ...
          rawAnalyzerSamples.push(
            new DataSample("", rawFlowTotals, "", "", "Flow", 0)
          );
          rawAnalyzerSamples.sort(function (a, b) {
            return a.displayIndex - b.displayIndex;
          });

          this.methaneTrend = buildTrend(
            rawAnalyzerSamples,
            "C1",
            "methaneChart",
            startISO,
          ...
          buildRawOutlierTable(
            rememberOutliers,
            startISO,
            endISO,
            rawAnalyzerSamples,
            rememberGasComponentOutliers
          );
    ...
    function buildRawOutlierTable(
      outliersFound,
      loadingStart,
      loadingEnd,
      myArray,
      gasComponentOutliers
    ) {
      var rawFlowTotalsTable = $(
        '<table id="rawFlowTotalsTable" class="table"></table>'
      );
      ...
      for (var i = 0; i < myArray[0].samples.length; i++) {
        ...
        var endDate = new Date(loadingEnd);
        var thisTime = new Date(time);
        if (myArray[0].samples[i].Value > flowLimit) {
          if (+thisTime >= +startDate && +thisTime <= +endDate) {
            ...
            for (var j = 0; j < myArray.length; j++) {
              var valWith2Decimals = truncateWith2Decimals(
                myArray[j].samples[i].Value
              );
              var foundGcOutlier = gasComponentOutliers.some(
                (outlier) =>
                  outlier.timestamp === myArray[j].samples[i].Timestamp &&
                  outlier.component === "CAM:LNG_Ship." + myArray[j].mapToName
              );
              if (foundGcOutlier) {
                rawTableCol +=
                  "<td style='background-color:#FF0000 !important;-webkit-print-color-adjust:exact;'>" +
                  valWith2Decimals +
                  "</td>";
              } else {
    ...
    function calculateAutoCovariance(start, end) {
      ...
      for (var i = 0; i < flowSamples.length; i++) {
        if (flowSamples[i].Timestamp >= start) {
          ...
          var autocovariance = GrubbsCalculator.calculateAutoCovariance(
            currentRange,
            prevRange
          );
          var sample = {
    ...
    function getOneHourRange(samples, currentTime) {
      ...
      var oneHourRange = samples.filter(function (sample) {
        return (
          sample.Timestamp >= oneHourBefore && sample.Timestamp <= currentTime
        );
      });
      return oneHourRange;
    }
    ...
    function getJettyTags(jetty, analyzer) {
      return SampledData.piTags.filter(function (tag) {
        return (
          (tag.Jetty == jetty && tag.Analyzer == analyzer) ||
          tag.TagType == "Flow"
        );
      });
    }

    function removeJunk(flowLimit) {
      ...
      do {
        ...
        dataSamples.forEach((set) => {
          ...
          set.samples.forEach((sample) => {
            ...
            if (set.sampleType == "Flow" && sample.Value < flowLimit) {
              console.log("outlier! MapTo NAME: " + gcName);
              outliers = outliers.concat(copyFlowSamples[i].Timestamp);
              rememberOutliers = rememberOutliers.concat(
                copyFlowSamples[i].Timestamp
              );
        ...
        if (outliers.length > 0) {
          dataSamples.forEach(function (set) {
            set.samples = GrubbsCalculator.removeOutliers(
              set.samples,
              outliers
            );
          });
        ...
        dataSamples.forEach(function (set) {
          if (set.sampleType == "Flow") {
            ...
            set.standardDeviation =
              GrubbsCalculator.calculateStandardDeviation_Sample(
                set.samples,
                set.average
              );
            var significanceLevel = 0.15;

            set.t_TestSignificance =
              GrubbsCalculator.calculateT_TestSignificance(
                set.samples,
                significanceLevel
              );

            var probability = 1 - set.t_TestSignificance;
            console.log(
              "prob: " +
                probability +
                " grubbsVal: " +
            ...
            set.grubbsTestValue = GrubbsCalculator.calculateGrubbsValue(
              set.size,
              probability
            );
            var setOutliers = new Array();
            setOutliers = GrubbsCalculator.identifyOutliers(
              set.samples,
              set.grubbsTestValue,
              set.average,
              set.standardDeviation
    ...
    function performCalculations() {
      ...
      do {
        ...
        dataSamples.forEach(function (set) {
          ...
          set.t_TestSignificance = GrubbsCalculator.calculateT_TestSignificance(
            set.samples,
            significanceLevel
          );
          ...
          gasComponentOutliers = GrubbsCalculator.identifyComponentOutliers(
            set.samples,
            set.grubbsTestValue,
            set.average,
            set.standardDeviation,
          ...
          if (gasComponentOutliers.length > 0) {
            ...
            rememberGasComponentOutliers =
              rememberGasComponentOutliers.concat(gasComponentOutliers);
          }
        });
        if (outliers.length > 0) {
          dataSamples.forEach(function (set) {
    ...
    function calculateNormalizedAverages() {
      dataSamples.forEach(function (set) {
        if (analyzerTags.indexOf(set.tag) >= 0 && set.mapTo.slice(-2) != "C1") {
          var normalizedAverage =
            (set.roundedAverage / roundedAverageTotal) * 100;
    ...
    function buildTable() {
      ...
      $(
        '<tr class="thead-dark"><th scope="col"></th><th scope="col"></th><th scope="col">' +
          dataSamples[0].samples.length +
          " remaining samples</th></tr>"
      ).appendTo(mytable);
      $(
        '<tr class="thead-dark"><th scope="col">Tag</th><th scope="col">Rounded</th><th scope="col">Normalized</th></tr>'
      ).appendTo(mytable);
      for (var i = 0; i < resultingData.tagSamples.length; i++) {
        var name = resultingData.tagSamples[i].name;
        var gcName = name.substring(name.indexOf(".") + 1);
        var roundedValWith2Decimals = truncateWith2Decimals(
          resultingData.tagSamples[i].rounded
        );
        var normalizedValWith2Decimals = truncateWith2Decimals(
          resultingData.tagSamples[i].normalized
        );
        $(
          '<tr scope="row"><td scope="col" col width="230">' +
            gcName +
            '</td><td scope="col">' +
            roundedValWith2Decimals +
            '</td><td scope="col">' +
            normalizedValWith2Decimals +
            "</td></tr>"
        ).appendTo(mytable);
      }
      var roundedValWith2Decimals = truncateWith2Decimals(
        resultingData.roundedAverageTotal
      );
      var normalizedValWith2Decimals = truncateWith2Decimals(
        resultingData.normalizedAverageTotal
      );
      $(
        '<tr scope="row"><td scope="col">Total</td><td scope="col">' +
          roundedValWith2Decimals +
          '</td><td scope="col">' +
          normalizedValWith2Decimals +
          "</td></tr>"
